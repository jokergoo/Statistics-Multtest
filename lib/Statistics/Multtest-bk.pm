...
sub _qvalue {
    my $p = shift;
    my %setup = ('lambda'         => seq(0, 0.91, 0.05),
                 'pi0_method'     => 'smoother',
                 'robust'         => 0,
                 'smooth_df'      => 3,
                 'smooth_log_pi0' => 0,
    ...
    my $pi0_method = $setup{'pi0_method'};
    my $robust = $setup{'robust'};
    my $smooth_df = $setup{'smooth_df'};
    my $smooth_log_pi0 = $setup{'smooth_log_pi0'};
	...
	if($pi0_method ne 'smoother' and $pi0_method ne 'bootstrap') {
		croak "ERROR: 'pi0.method' must be one of 'smoother' or 'bootstrap'.";
	}
	if($smooth_df != 3) {
		croak "ERROR: Only cubic spline interpolation (smooth_df = 3) is supported.";
	}
    ...
    if(len($lambda) == 1) {
		...
		$pi0 = mean(test($p, sub {$_[0] >= $lambda})) / (1 - $lambda);
		# pi0 <- min(pi0,1)
		$pi0 = min(c($pi0, 1));
	}
	else {
...
#		if($pi0_method eq 'smoother') {
#		
#			# if(smooth.log.pi0)
#			#     pi0 <- log(pi0)
#			if($smooth_log_pi0) {
#				$pi0 = sapply($pi0, sub {log($_[0])});
#			}
#			# spi0 <- smooth.spline(lambda,pi0,df=smooth.df)
#			#my $spi0 = smooth_spline($lambda, $pi0, $smooth_df);
#			my $spi0 = new Math::Spline($lambda, $pi0);
#
#			# pi0 <- predict(spi0,x=max(lambda))$y
#			#$pi0 = predict_on_y($spi0, max($lambda));
#			$pi0 = $spi0->evaluate(max($lambda));
#			
#			# if(smooth.log.pi0)
#			#     pi0 <- exp(pi0)
#			if($smooth_log_pi0) {
#				$pi0 = sapply($pi0, sub {exp($_[0])});
#			}
#			# pi0 <- min(pi0,1)
#			$pi0 = min(c($pi0, 1));
#		
#		}
		if($pi0_method eq 'bootstrap') {
			# minpi0 <- min(pi0)
			my $minpi0 = min($pi0);
			# mse <- rep(0,length(lambda))
			my $mse = rep(0, len($lambda));
			# pi0.boot <- rep(0,length(lambda))
			my $pi_boot = rep(0, len($lambda));
			for (1..100) {
				...
				# }
				my $pi0_boot = sapply(seq(0, len($lambda) - 1),
								      sub {
									      my $current_lambda = $lambda->[$_[0]];
			...
			my $min_mse = min($mse);
			$pi0 = min(subset($pi0, which(test($mse, sub {$_[0] == $min_mse}))));
			$pi0 = min(c($pi0, 1));
		}
    }
    ...
    # }
    for my $i (($#$u-1)..0) {
        $qvalue->[$u->[$i]] = min(c($qvalue->[$u->[$i]], $qvalue->[$u->[$i+1]], 1));
    }
...
sub _approximately_compare {
	my $left = shift;
	my $right = shift;
	my $bigger = shift;  # 1 for '>', '>=' and 0 for '<', '<='
	
	if($bigger eq ">" or $bigger eq ">=") {
		$bigger = 1;
	}
	elsif($bigger eq "<" or $bigger eq "<=") {
		$bigger = 0;
	}
...
C<%setup> are as follows

  lambda         => seq(0, 0.90, 0.05),  # should be an array reference
  pi0_method     => 'smoother',          # 'smoother' or 'bootstrap'
  robust         => 0,                   # true or false
  smooth_df      => 3,                   # only 3 is allowed here
  smooth_log_pi0 => 0,                   # true or false
